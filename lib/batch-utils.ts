...
export async function processBatch<T, R>(
  ...
  const executeTask = async (item: T, index: number) => {
    try {
      ...
      console.error(`[BatchUtils] Task ${index} failed:`, err.message);
    } finally {
      completed++;
      if (onProgress) {
...
export async function retryTask<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(`[BatchUtils] Task failed (attempt ${attempt}/${maxRetries}):`, lastError.message);
...
export function createProgressLogger(taskName: string) {
  ...
  return (completed: number, total: number) => {
    ...
    if (now - lastLogTime >= logInterval || completed === total) {
      ...
      console.log(`[${taskName}] Progress: ${completed}/${total} (${percentage}%)`);
      lastLogTime = now;
    }
  };
}
