...
const CPASS_SERVICES = [
  'DHL_EXPRESS_WORLDWIDE',
  'FEDEX_INTL_CONNECT_PLUS',
  'FEDEX_INTL_ECONOMY', 
  'FEDEX_INTL_PRIORITY',
  'SPEEDPAK_ECONOMY',
  'SPEEDPAK_EXPRESS_DHL',
  'UPS_EXPRESS_SAVER'
]

/**
 * 国コード変換機能（実際のDBデータに基づく）
 */
...
async function getFuelSurcharge(carrierId: string, basePrice: number): Promise<number> {
  try {
    ...
    if (surcharge && surcharge.surcharge_rate) {
      return basePrice * (Number(surcharge.surcharge_rate) / 100)
    }
    return 0
  } catch (error) {
...
export async function getCPassRates(
  ...
  try {
    ...
    console.log('変換後の国コード:', countryCodes, '重量:', weightKg)
    ...
    if (error) {
      ...
      return { error: { message: 'CPass料金の取得に失敗しました: ' + JSON.stringify(error), timestamp: new Date().toISOString() } }
    }
    ...
    if (data && data.length > 0) {
      for (const rate of data) {
        // 安全性チェック
        if (!rate.cpass_services || !rate.cpass_countries) {
          console.warn('不完全なデータをスキップ:', rate.id)
          continue
        }
    ...
    return { error: { message: 'CPass料金の取得中にエラーが発生しました: ' + (error as Error).message, timestamp: new Date().toISOString() } }
  }
}

/**
 * 日本郵便料金取得（国フィルタリング対応版）
 */
export async function getJPPostRates(
  ...
  try {
    ...
    console.log('重量条件:', weightG, '国コード:', countryCode)
    ...
    if (zoneError) {
      console.error('JP Post zone lookup error:', zoneError)
      return { error: { message: '日本郵便ゾーンの取得に失敗しました: ' + JSON.stringify(zoneError), timestamp: new Date().toISOString() } }
    }

    if (!countryZones || countryZones.length === 0) {
      console.log(`国 ${countryCode} に対応するゾーンが見つかりません`)
      return { data: [] }
    }

    const zoneIds = countryZones.map(cz => cz.zone_id)
    console.log('対象ゾーンIDs:', zoneIds)
    ...
    console.log('日本郵便取得データ件数:', data?.length || 0)
    if (data && data.length > 0) {
    ...
    if (data && data.length > 0) {
      for (const rate of data) {
        // 安全性チェック
        if (!rate.shipping_services || !rate.shipping_zones) {
          console.warn('不完全な日本郵便データをスキップ:', rate.id)
          continue
        }
...
export async function getElojiRates(
  ...
  try {
    ...
    console.log('Eloji - 国コード:', countryCodes, '重量:', weightKg)
    ...
    if (error) {
      ...
      return { error: { message: 'Eloji料金の取得に失敗しました: ' + JSON.stringify(error), timestamp: new Date().toISOString() } }
    }
    ...
    if (data && data.length > 0) {
      for (const rate of data) {
        // 安全性チェック
        if (!rate.cpass_services || !rate.cpass_countries) {
          console.warn('不完全なElojiデータをスキップ:', rate.id)
          continue
        }
    ...
    return { error: { message: 'Eloji料金の取得中にエラーが発生しました: ' + (error as Error).message, timestamp: new Date().toISOString() } }
  }
}
...
export async function calculateShipping(
  ...
  try {
    ...
    console.log('業者別内訳:', allResults.reduce((acc, r) => {
...
export async function generateMatrixData(
  ...
  try {
    ...
    console.log('対象国コード:', countryCodes)
    
    const allServices: any[] = []
    
    // 各重量での全サービス料金を取得
    for (const weight of weights) {
      console.log(`重量 ${weight}kg の料金取得中...`)
      
      // CPass料金（Eloji除外）
      const { data: cpassRates, error: cpassError } = await supabase
        .from('cpass_rates')
        .select(`
          rate_jpy,
          cpass_services!inner(
      ...
      if (cpassError) {
        console.error(`CPass ${weight}kg エラー:`, cpassError)
      }
        
      // Eloji料金（DHL, FedX, UPS）
      const { data: elojiRates, error: elojiError } = await supabase
        .from('cpass_rates')
        .select(`
          rate_jpy,
          cpass_services!inner(
      ...
      if (elojiError) {
        console.error(`Eloji ${weight}kg エラー:`, elojiError)
      }
        
      // 日本郵便料金
      const { data: jpRates, error: jpError } = await supabase
        .from('shipping_rates')
        .select(`
          base_price_jpy,
          shipping_services!inner(
      ...
      if (jpError) {
        console.error(`日本郵便 ${weight}kg エラー:`, jpError)
      }
      
      // CPassデータ追加（NULL安全性チェック）
      cpassRates?.forEach(rate => {
        if (rate.cpass_services && rate.cpass_countries) {
          allServices.push({
            carrier_name: 'CPass',
            carrier_code: 'CPASS',
            service_name: rate.cpass_services.service_name_ja || rate.cpass_services.service_name_en || '不明なサービス',
            service_code: rate.cpass_services.service_code,
      ...
      // 日本郵便データ追加（NULL安全性チェック）
      jpRates?.forEach(rate => {
        if (rate.shipping_services && rate.shipping_services.shipping_carriers) {
          allServices.push({
            carrier_name: '日本郵便',
            carrier_code: 'JPPOST',
            service_name: rate.shipping_services.service_name || '不明なサービス',
            service_code: rate.shipping_services.service_code,
            service_type: rate.shipping_services.service_type || 'standard',
            price_jpy: Number(rate.base_price_jpy),
            price_usd: Number(rate.base_price_jpy) / USD_JPY_RATE,
            weight_g: weight * 1000,
            country_code: country,
            available: true
          })
    ...
    console.log('総サービス件数:', allServices.length)
    ...
    console.log('グループ化後のサービス数:', Object.keys(serviceGroups).length)
    ...
    return {
      ...
      meta: { 
        execution_time_ms: Date.now(),
        services_count: Object.keys(serviceGroups).length,
      ...
      carriers: [
        {
          carrier_code: 'CPASS',
          carrier_name: 'CPass',
          services_count: servicesRes.data?.filter(s => !ELOJI_SERVICES.includes(s.service_code)).length || 7,
        ...
        },
        {
          ...
          carrier_name: 'FedX',
          services_count: 3,
          rates_count: Math.floor((cpassCountRes.count || 0) * 0.3),
        ...
        },
        {
          ...
          carrier_name: 'UPS',
          services_count: 1,
          rates_count: Math.floor((cpassCountRes.count || 0) * 0.1),
          cheapest_price_jpy: 2980,
