...
export async function POST(request: NextRequest) {
  try {
    ...
    // RPC関数を使用して承認
    const { error: rpcError } = await supabase.rpc('approve_ai_message', {
    ...
    if (rpcError) {
      ...
      return NextResponse.json(
        { success: false, error: 'メッセージの承認に失敗しました' },
        { status: 500 }
      )
    }
    ...
    console.log('✅ AI Message approved:', message_id)
    ...
    console.error('❌ POST /api/tools/messages/approve error:', error)
    return NextResponse.json(
