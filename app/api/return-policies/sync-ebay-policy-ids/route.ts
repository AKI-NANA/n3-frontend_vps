...
const EBAY_API_URL = 'https://api.ebay.com/sell/account/v1/return_policy';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`[${account}] eBay Return Policy取得開始...`);
    ...
    const ebayPolicies: EbayPolicy[] = ebayData.returnPolicies || [];

    console.log(`[${account}] eBayから ${ebayPolicies.length} 件のReturn Policyを取得`);

    const policyMap = new Map<string, string>();
    ebayPolicies.forEach((policy) => {
      policyMap.set(policy.name, policy.returnPolicyId);
    });
    ...
    console.log(`[${account}] データベースから ${dbPolicies?.length || 0} 件のReturn Policyを取得`);
    ...
    console.log(`[${account}] Return Policy完了: 成功=${successCount}, 失敗=${errorCount}, 新規=${newPolicies}`);
    ...
    console.error('Return Policy同期エラー:', error);
    return NextResponse.json(
      {
        error: 'Return Policy同期に失敗しました',
