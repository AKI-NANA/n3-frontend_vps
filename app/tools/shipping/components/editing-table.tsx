...
// ===== CURRENCY =====
function Currency({ value, currency = 'JPY' }: { value?: number | null; currency?: string }) {
  if (value == null) return <span className="text-gray-400 text-[11px]">-</span>
  const formatted = currency === 'JPY' ? `¥${value.toLocaleString()}` : `$${value.toFixed(2)}`
  return <span className="font-mono text-[11px] text-gray-700">{formatted}</span>
}
