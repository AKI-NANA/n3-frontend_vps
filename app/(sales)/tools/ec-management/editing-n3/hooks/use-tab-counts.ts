    ...
    try {
      ...
      if (data.success) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('[useTabCounts] v3 カウント:', {
          ...
          console.log('[useTabCounts] 工程フィルター:', {
            translation: data.counts.workflow_translation,
            search: data.counts.workflow_search,
            selection: data.counts.workflow_selection,
            details: data.counts.workflow_details,
            enrichment: data.counts.workflow_enrichment,
            approval: data.counts.workflow_approval,
            listed: data.counts.workflow_listed,
            others: data.counts.workflow_others,
          });
        }
      } else {
