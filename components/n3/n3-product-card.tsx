...
import { Check, AlertTriangle, FileText, Truck, Globe, Tag, Loader2 } from 'lucide-react';
...
export interface N3ProductCardProps {
  ...
  /** フィルター不合格の理由（例: 'VERO: MTG'） */
  filterFailReason?: string;
  /** VERO検出ブランド名 */
  veroDetectedBrand?: string;
  category?: string;
...
export const N3ProductCard = memo(function N3ProductCard({
  ...
  return (
    ...
    >
      ...
      >
        {/* ローディングインジケータ */}
        {isInView && !imageLoaded && !imageError && imageUrl && (
          <div
            style={{
              position: 'absolute',
              top: 0,
        ...
        {isVeroBrand && (
          ...
          >
            {humanApproved ? <Check size={10} /> : <AlertTriangle size={10} />}
            {humanApproved ? '承認済' : 'VERO'}
          </div>
        )}
        
        {/* フィルター不合格理由（VEROブランドで未承認の場合のみ表示） */}
        {filterPassed === false && filterFailReason && !humanApproved && (
          <div
            style={{
              position: 'absolute',
              bottom: 28,
