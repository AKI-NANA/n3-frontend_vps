...
# - node_modules: 依存関係（VPSでインストール） ← 重要！
# - 開発用ツール設定: .vscode, .idea等
# - バックアップ: _backups, backups
# - ドキュメント: docs, *.md（READMEなど）
# - テスト: __tests__（本番不要）
# - .DS_Store: MacOSの不要ファイル
EXCLUDES_ESSENTIAL="--exclude=.git --exclude=.next --exclude=node_modules --exclude=.vscode --exclude=.claude --exclude=.vercel --exclude=.idea --exclude=_backups --exclude=_src --exclude=backups --exclude=docs --exclude=__tests__ --exclude=.DS_Store --exclude=*.log --exclude=*.backup --exclude=*.bak"
...
sync_vps() {
  ...
  echo "📝 除外設定: node_modules, .git, .next, .DS_Store 等"
